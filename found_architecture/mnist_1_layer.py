...
C[K:2-F:82-L2:0.0018]
C[K:2-F:100-L2:0.0001]
C[K:2-F:100-L2:0.0001]
P[K:2-S:2]
'''
...
def model():
    ...
    with slim.arg_scope([slim.conv2d, slim.fully_connected], activation_fn=tf.nn.crelu, normalizer_fn=slim.batch_norm, normalizer_params={'is_training': is_training, 'decay': 0.95}):
        h = slim.conv2d(inputs=x, num_outputs=100, kernel_size=2, weights_regularizer=slim.l2_regularizer(0.0010))
        h = slim.conv2d(inputs=h, num_outputs=82, kernel_size=2, weights_regularizer=slim.l2_regularizer(0.0018))
        h = slim.conv2d(inputs=h, num_outputs=100, kernel_size=2, weights_regularizer=slim.l2_regularizer(0.0001))
        h = slim.conv2d(inputs=h, num_outputs=100, kernel_size=2, weights_regularizer=slim.l2_regularizer(0.0001))
